...
CDrawObj::CDrawObj(const CRect& position ,const CString& objName)//ZDO
{
	m_position = position;
	m_pDocument = NULL;
	ourObjName = objName; //ZDO
...
CDrawObj* CDrawObj::Clone(CDrawDoc* pDoc)
{
	...
	CDrawObj* pClone = new CDrawObj(m_position,ourObjName);
...
CDrawPoly::CDrawPoly(const CRect& position)
	: CDrawObj(position,(CString)"quxiao")//ZDO
{
	m_points = NULL;
	m_nPoints = 0;
	m_nAllocPoints = 0;
